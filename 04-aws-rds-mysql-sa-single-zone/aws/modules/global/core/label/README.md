...
This will create an `id` with the value of `eg-prod-backend-public` because when generating `id`, the default order is `namespace`, `environment`, `stage`,  `name`, `attributes`
...
```hcl
resource "aws_instance" "eg_prod_backend_public" {
  instance_type = "t1.micro"
  tags          = module.eg_prod_backend_label.tags
}
```
...
```hcl
resource "aws_security_group" "eg_prod_backend_public" {
  vpc_id = var.vpc_id
  name   = module.eg_prod_backend_label.id
  tags   = module.eg_prod_backend_label.tags
  egress {
    from_port   = 0
    to_port     = 0
...
```hcl
module "eg_prod_backend_abc_label" {
  source     = "git::https://github.com/cloudposse/terraform-null-label.git?ref=master"
  namespace  = "eg"
  stage      = "prod"
  name       = "backend"
...
resource "aws_security_group" "eg_prod_backend_abc" {
  name = module.eg_prod_backend_abc_label.id
  tags = module.eg_prod_backend_abc_label.tags
  ingress {
    from_port   = 22
    to_port     = 22
    protocol    = "tcp"
...
resource "aws_instance" "eg_prod_backend_abc" {
   instance_type          = "t1.micro"
   tags                   = module.eg_prod_backend_abc_label.tags
   vpc_security_group_ids = [aws_security_group.eg_prod_backend_abc.id]
}

module "eg_prod_backend_xyz_label" {
  source     = "git::https://github.com/cloudposse/terraform-null-label.git?ref=master"
  namespace  = "eg"
  stage      = "prod"
  name       = "backend"
...
resource "aws_security_group" "eg_prod_backend_xyz" {
  name = module.eg_prod_backend_xyz_label.id
  tags = module.eg_prod_backend_xyz_label.tags
  ingress {
    from_port   = 22
    to_port     = 22
    protocol    = "tcp"
...
resource "aws_instance" "eg_prod_backend_xyz" {
   instance_type          = "t1.micro"
   tags                   = module.eg_prod_backend_xyz_label.tags
   vpc_security_group_ids = [aws_security_group.eg_prod_backend_xyz.id]
}
```
